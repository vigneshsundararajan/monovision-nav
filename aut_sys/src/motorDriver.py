...
class motorDriver:
  ...
  def init_app(self):
    ...
    rospy.Subscriber('motors',motors,self.__motorUpdate_cbk)
    rospy.init_node('motorDriver',anonymous=True)
    self.rate = rospy.Rate(10)
  
  def __motorUpdate_cbk(self, data):#updata motor speeds and directions
    # Determine the values to output using the mux
    if data.leftSpeed > 0:
      aL = 0b1
      bL = 0b0
    else:
